...
tf.app.flags.DEFINE_integer("per_checkpoint", 1000,
                            "How many steps to do per checkpoint.")
tf.app.flags.DEFINE_integer("inference_version", 0,
                            "The version for inferencing.")
...
def gen_batch_data(data):
    ...
    batched_data = {'texts': np.array(
        texts), 'texts_length': texts_length, 'labels': labels}
...
def evaluate(model, sess, dataset):
    ...
    while ed < len(dataset):
        ...
        outputs = sess.run(['loss:0', 'accuracy:0'], {
                           'texts:0': batch_data['texts'], 'texts_length:0': batch_data['texts_length'], 'labels:0': batch_data['labels']})
        loss += outputs[0]
        accuracy += outputs[1]
    return loss / len(dataset), accuracy / len(dataset)
